...
class Tagger(object):
    ...
    def __init__(self, sentences):
        ...
        vocab["<UNK>"]=[0 for i in range(len(self.l_tags))]
        for word in vocab:
    ...
    def most_probable_tags(self, tokens):
        ...
        for i in tokens:
            if i in self.emission_prob:
                ...
                max = 0
                temp = -1
                for j in range(len(self.emission_prob["<UNK>"])):
                    if self.emission_prob["<UNK>"][j] >= max:
                        max = self.emission_prob["<UNK>"][j]
                        temp = j
                result.append(self.l_tags[temp])
    ...
    def viterbi_tags(self, tokens):
        ...
        for i in range(len(l[0])):
            ...
            if tokens[0] in self.emission_prob:
                ...
                l[0][i].value = self.tags_init_prob[self.l_tags[i]] * self.emission_prob["<UNK>"][i]
        ...
        for i in range(1,len(l)):
            for j in range(len(l[i])):
                ...
                l[i][j].prev=node
                if tokens[i] in self.emission_prob:
                    l[i][j].value = max * self.emission_prob[tokens[i]][j]
                else:
                    l[i][j].value = max * self.emission_prob["<UNK>"][j]
